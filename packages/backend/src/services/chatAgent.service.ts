...
function buildPostConsentFallbackReply(nextField: NextField | null): ChatReply {
  const next = nextField?.fieldKey === 'business.businessStartDate'
    ...
    : nextField?.fieldKey === 'application.homeBasedBusiness'
      ? 'Great! Next up is Business Details. Take a quick look at the information we found, and if anything looks off, use the Edit button to fix it. Also, is this a home-based business? Answer Yes or No on screen (or tell me here).'
      : 'Great! Next up is Business Details. Take a quick look at the information on screen and confirm anything we found. If something looks off, click Edit to correct it, then Confirm & Continue.';
...
async function requestOpenAiReply(input: {
  ...
  const systemPrompt = [
    ...
    'Tone: warm, friendly, encouraging, professional, concise (typically 1-3 short paragraphs), conversational, like a knowledgeable human funding specialist. Celebrate progress naturally (for example "Great, that\'s done!" or "You\'re almost there!"). Acknowledge what the merchant said before guiding them forward. Ask one useful follow-up question at a time. Vary your wording so consecutive replies do not look alike.',
    'Style rule: NEVER use em dashes or en dashes in your replies. Use a comma, period, colon, or parentheses instead.',
